...
Runner.prototype._success = function (step) {
	...
	var data
	if (typeof arguments[1] === 'number' && arguments[1] > 199) {
		this._HTTPStatusCode = arguments[1]
		data = [].slice.call(arguments, 2)
	} else {
...
Runner.prototype._end = function (err, data) {
	...
	// Add HTTP status code to data
	if (this._HTTPStatusCode) {
		data[0].HTTPStatusCode = this._HTTPStatusCode
	}
...
Runner.prototype._createError = function (success) {
	 ...
	 * Four uses:
	 * error(fn) -> Function
	 * error(str, ?obj) or error(code, str, ?obj)
	 * error(x, ?obj)
	 * error(HTTPStatusCode, str, ?obj)
	 */
	var error = function () {
		var len = arguments.length,
			msg, errorObj
		if (len === 1 && typeof arguments[0] === 'function') {
			...
			Object.assign(errorObj, arguments[1])
			throw errorObj
			...
			Object.assign(errorObj, arguments[2])
			throw errorObj
		} else if (len > 2 && that._enableErrorCode && typeof arguments[2] === 'string') {
			// error (HTTPStatusCode, code, str, ?obj)
			msg = arguments[2]
			errorObj = new that._errorClass(msg)
			errorObj.HTTPStatusCode = arguments[0]
			errorObj.code = arguments[1]
			Object.assign(errorObj, arguments[3])
			throw errorObj
		} else {
			// error(x, ?obj)
